...
let Settings = {
	...
	About: () => {
		return '<hr>' +
			...
			'<p>' + i18n('Settings.About.TranslateDesc') + '</p>' +
			'<hr>' +
	...
	LGInvestmentStepsField: () => {
		let ip = $('<input />').addClass('setting-input').attr({
					type: 'text',
					id: 'lg-investment-steps',
					step: 1,
					min: 2
				}),
			value = Settings.GetSetting('LGInvestmentSteps');
		
		if(null !== value){
			ip.val(value.join(','));
			ip.attr({value: value.join(',')});
		}
		
		$('#SettingsBox').on('blur', '#lg-investment-steps', function(){
			let value = $(this).val();
			
			value = value.trim();

			if(value.length > 0){
				if (null !== value.match(/^(\d{2}(.\d)?|\d{3})(,(\d{2}(.\d)?|\d{3}))*$/)) {
					localStorage.setItem('LGInvestmentSteps', JSON.stringify(value.split(',')));
				}
			} else {
				localStorage.removeItem('LGInvestmentSteps');
			}
		});
